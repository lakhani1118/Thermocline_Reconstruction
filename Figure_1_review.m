...
latvec=latvec(:);
lonvec=cycleLon(:,1);
clear X3 Y3 Z3 d18O1
[~,I5]=min(abs((lonvec-180)));
res=5;
for j=1:30
    for i=234:394
        cycled18O(I5-res:I5+res,i,j)=interp1([lonvec(I5-res),lonvec(I5+res)],[squeeze(cycled18O(I5-res,i,j)),squeeze(cycled18O(I5+res,i,j))],lonvec(I5-res:I5+res));
    end
end
%%
ACD=[17,210,114,94];
background=ones([4,length(lonvec),length(latvec)])*nan;
for k=1:length(ACD)
    for i=1:length(lonvec)
        for j=1:length(latvec)
            background(k,i,j)=interp1(depthvec(1:27),squeeze(cycled18O(i,j,1:27)),ACD(k));
        end
    end
end
